...
class Templates(SearchableMixin, PaginatedAPIMixin, db.DynamicDocument):
    """
    ---------------------------
    ...
    def get_schema():
        return {
            ...
            "templates": {
                ...
                "name": "Name",
                "description": "Description",
                "created_datetime": "Create Datetime",
...
class Banners(SearchableMixin, PaginatedAPIMixin, db.DynamicDocument):
    ...
    pages = db.ListField(ReferenceField(Pages, reverse_delete_rule=CASCADE))
    ...
    def get_schema():
        return {
            ...
            "sc": 0,
            "order": ["banner_id", "header", "text", "image"],
        }

    def __repr__(self):
        return "<Banner {}>".format(self.banner_id)


class Partners(SearchableMixin, PaginatedAPIMixin, db.DynamicDocument):
    """
    -----------------------
    ...
    partner_id = db.IntField(unique=True, index=True)
    partner_name = db.StringField(unique=True, index=True)
    description = db.StringField()
    partner_image = ReferenceField(Images, reverse_delete_rule=CASCADE)
    meta = {
        "db_alias": "default",
        "collection": "Partners",
        "indexes": [
    ...
    def __repr__(self):
        return "<Partners {}>".format(self.partner_name)


class Ratings(SearchableMixin, PaginatedAPIMixin, db.DynamicDocument):
    """
    Defines Ratings
    """

    rating_id = db.IntField(unique=True, index=True)
    name = db.StringField(unique=True)
    percentage = db.IntField()
    description = db.StringField()
    ...
    def __repr__(self):
        return "<Ratings {}>".format(self.rating_id)


class Jobs(SearchableMixin, db.DynamicDocument):
    """
    ----------------------------
...
class ServiceTypes(SearchableMixin, PaginatedAPIMixin, db.DynamicDocument):
    """
    ---------------------
    Service Types
    ---------------------
    ...
    type_description = db.StringField()
    meta = {
        "db_alias": "default",
    ...
    def __repr__(self):
        return "<ServiceTypes {}>".format(self.type_name)


class Sections(SearchableMixin, PaginatedAPIMixin, db.DynamicDocument):
    """
    -----------------
    Define Sections
    -----------------
    ...
    section_id = db.IntField(unique=True, index=True)
    name = db.StringField(unique=True, index=True)
    description = db.StringField()
    pages = db.ListField(ReferenceField(Pages, reverse_delete_rule=CASCADE))
    meta = {
    ...
    def __repr__(self):
        return "<Sections {}>".format(self.name)


class TeamMembers(SearchableMixin, PaginatedAPIMixin, db.DynamicDocument):
    """
    -------------------
    Define TeamMembers
    -------------------
    ...
    member_id = db.IntField(unique=True, index=True)
    name = db.StringField(unique=True, index=True)
    role = db.StringField()
    description = db.StringField()
    ...
    def __repr__(self):
        return "<TeamMembers {}>".format(self.name)


class Services(SearchableMixin, PaginatedAPIMixin, db.DynamicDocument):
    """
    ------------------
    Define Services
    ------------------
    ...
    MONTHLY = 1
    YEARLY = 2
    service_id = db.IntField(unique=True, index=True)
    name = db.StringField(unique=True, index=True)
    description = db.StringField()
    price = db.DecimalField()
    currency = db.StringField()
    period = db.IntField(choices=[WEEKLY, MONTHLY, YEARLY])
    available_features = db.ListField()
    restricted_features = db.ListField()
    image = db.StringField()  # ReferenceField(Images, reverse_delete_rule=CASCADE)
    service_type = ReferenceField(ServiceTypes, reverse_delete_rule=CASCADE)
    page = ReferenceField(Pages, reverse_delete_rule=CASCADE)
    meta = {
        "db_alias": "default",
        "collection": "Services",
        "indexes": [
    ...
    def get_schema():
        return {
            ...
            "services": {
                ...
                "restricted_features": "Restricted Features",
                "created_datetime": "Create Datetime",
                "last_modified_date": "Last Modified Date",
            },
            "sc": 0,
...
class Faqs(SearchableMixin, PaginatedAPIMixin, db.DynamicDocument):
    """
    --------------------------
    ...
    faq_id = db.IntField(unique=True, index=True)
    question = db.StringField()
    inquirer = db.DecimalField()
    response = db.StringField()
    responder = db.ReferenceField(Users, reverse_delete_rule=CASCADE)
    time_asked = db.DateTimeField(default=datetime.now())
    time_responded = db.DateTimeField()
    ...
    def get_schema():
        return {
            ...
            "Services": {
                ...
                "inquirer": "inquirer",
                "response": "response",
                "time_asked": "Time Asked",
                "time_responded": "Time Responded",
                "created_datetime": "Created Datetime",
                "last_modified_date": "Last Modified Date",
            },
            "sc": 0,
    ...
    def __repr__(self):
        return "<Faqs {}>".format(self.faq_id)
...
class SiteSettings(SearchableMixin, PaginatedAPIMixin, db.DynamicDocument):
    ...
    meta = {
        ...
        "indexes": [
            ("settings_id", "created_datetime", "last_modified_date", "current_version")
        ],
    }
...
class Messages(SearchableMixin, PaginatedAPIMixin, db.DynamicDocument):
    ...
    message_status = db.IntField(choices=[NEW, SEEN, ARCHIVED, DELETED])
    ...
    def __repr__(self):
        return "<Messages {}>".format(self.message_name)


class Clients(SearchableMixin, PaginatedAPIMixin, db.DynamicDocument):
    """
    ----------------------------------
    ...
    ACTIVE = 1
    DISABLED = 0
    client_id = db.IntField(unique=True, index=True)
    first_name = db.StringField(index=True)
    last_name = db.StringField(index=True)
    date_of_birth = db.DateField(index=True)
    email_address = db.StringField(unique=True, index=True)
    password = db.StringField()
    address = db.StringField(index=True)
    phone_number = db.StringField(index=True)
    status = db.IntField(default=ACTIVE, choices=[ACTIVE, DISABLED])
    profile_image = ReferenceField(Images, reverse_delete_rule=CASCADE)
    # orders = db.ListField() ## This should be derived from the orders table. No need to include it here.
    last_modified_date = db.DateTimeField(default=datetime.now())
    created_datetime = db.DateTimeField(default=datetime.now())
    current_version = db.IntField(default=0)
    meta = {
    ...
    def __repr__(self):
        return "<Clients {}>".format(self.email_address)


class ClientPersonalInformation(SearchableMixin, PaginatedAPIMixin, db.DynamicDocument):
    """
    ----------------
    Additional Client Personal Information
    ----------------
    """

    personal_info_id = db.IntField(unique=True, index=True)
    client = ReferenceField(Clients, reverse_delete_rule=CASCADE)
    gender = db.StringField(index=True)
    nationality = db.StringField()
    identification_id = db.StringField()
    mailing_address = db.StringField(index=True)
    education_history = db.ListField()
    employment_status = db.StringField()
    work_history = db.ListField()
    marital_status = db.StringField(
        choices=["Single", "Married", "Divorced", "Widow", "Widower"]
    )
    spouse_information = db.StringField()
    dependent_information = db.ListField()
    language_proficiency = db.ListField()
    disability = db.BooleanField(default=False)
    disabiliy_info = db.StringField()
    consent = db.BooleanField(default=False)

    meta = {
        "db_alias": "default",
        "collection": "ClientPersonalDetails",
        "indexes": [
            (
                "personal_info_id",
                "client",
    ...
    def get_schema():
        return {
            ...
            "clientpersonalinformation": {
                "personal_info_id": "Personal Information ID",
                "client": "Client ID",
                "gender": "Gender",
                ...
                "dependent_information": "Dependents",
    ...
    def __repr__(self):
        return "<ClientPersonalInformation {}>".format(self.personal_info_id)


class ClientTravelInformation(SearchableMixin, PaginatedAPIMixin, db.DynamicDocument):
    """
    ----------------
    ...
    travel_info_id = db.IntField(unique=True, index=True)
    client = ReferenceField(Clients, reverse_delete_rule=CASCADE)
    country_of_interest = db.StringField()
    immigration_status = db.StringField()
    desired_visa_type = db.StringField()
    immigration_reasons = db.StringField()
    passport_details = db.StringField()
    proof_of_funds = db.ListField()
    previous_visa_applications = db.ListField()
    travel_history = db.ListField()
    police_records = db.ListField()
    medical_records = db.ListField()
    insurance_information = db.ListField()
    documents = db.ListField()
    representative_information = db.StringField()
    consent = db.BooleanField(default=False)

    meta = {
        "db_alias": "default",
        "collection": "ClientTravelInformation",
        "indexes": [
            (
                "travel_info_id",
                "client",
    ...
    def get_schema():
        return {
            "titleField": "travel_info_id",
            "idField": "client_id",
            "clienttravelinformation": {
                "travel_info_id": " Travel Information ID",
                "client": "Client ID",
                "country_of_interest": "Country of Interest",
                "immigration_status": "Immigration Status",
                "desired_visa_type": "Desired Visa Type",
                "immigration_reasons": "Immigration Reasons",
                "passport_details": "Passport Details",
                "proof_of_funds": "Proof of Funds",
                "previous_visa_applications": "Visa Applications",
                "travel_history": "Travel History",
                "police_records": "Police Records",
                "medical_records": "Medical Records",
                "insurance_information": "Insurance Information",
                "documents": "Documents",
                "representative_information": "Representative",
                "consent": "Consent",
            },
            "sc": 0,
            "order": [
    ...
    def __repr__(self):
        return "<ClientTravelInformation {}>".format(self.travel_info_id)
...
class ClientServices(SearchableMixin, PaginatedAPIMixin, db.DynamicDocument):

    client_service_id = db.IntField(unique=True, index=True)
    client = ReferenceField(Clients, reverse_delete_rule=CASCADE)
    service = ReferenceField(Services, reverse_delete_rule=CASCADE)
    status = db.IntField()
    start_date = db.DateTimeField(default=datetime.now())
    end_date = db.DateTimeField(default=datetime.now())
    units = db.IntField()
    ...
    def get_schema():
        return {
            "titleField": "Client Service ID",
            "idField": "client_service_id",
            "clientservice": {
    ...
    def __repr__(self):
        return "<ClientServices {}>".format(self.client_service_id)
