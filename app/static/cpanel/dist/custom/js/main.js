...
if ($.fn.getObjectType(appConfig["image_formats"]) != "object") { 
    appConfig["image_formats"]= JSON.parse(appConfig["image_formats"])
}
  ...
  <div class="card card-primary" id="image-display">
    ...
    <div class="card-body">
      <div class="form-group row">
        <label for="report-name-id" class="form-label col-sm-1">Photo</label>
		        <img  class="col-sm-11 img-fluid" src="https://drive.google.com/thumbnail?id=${imageProps.google_id}" alt="${imageProps.file_name ? imageProps.file_name : ''}" />
      ...
      <div class="form-group row">
        <label for="file-name" class="form-label col-sm-1">Name</label>
        <input type="text" class="form-control col-sm-11" id="file-name" value="${imageProps.file_name ? imageProps.file_name : ''}" disabled="disabled">
      </div>
      <div class="form-group row">
        <label for="file-size" class="form-label col-sm-1">Size</label>
        <input type="text" class="form-control col-sm-11" id="file-size" value="${imageProps.file_size ? imageProps.file_size : ''} KB" disabled="disabled">
      </div>
      <div class="form-group row">
        <label for="image-dimensions" class="form-label col-sm-1">Dimensions</label>
        <input type="text" class="form-control col-sm-11" id="image-dimensions" value="(width,height)= ${imageProps.dimensions ? imageProps.dimensions : ''}" disabled="disabled">
      </div>

      <div class="form-group row">
        <label for="image-format" class="form-label col-sm-1" >Format</label>
        <input type="text" class="form-control col-sm-11" id="image-format" value="${imageProps.image_format ? imageProps.image_format : ''}" disabled="disabled">
      </div>

      <div class="form-group row">
        <label for="image-date" class="form-label col-sm-1">Date</label>
        <input type="text" class="form-control col-sm-11" id="image-date" value="${imageProps.created_datetime ? imageProps.created_datetime : ''}" disabled="disabled">
      </div>	  
...
$.fn.selectPlaceHolder = (elementId, cancelCallback, insertCallback) => {
  $.fn.resetModal();
  let modalOptions = {
    keyboard: false,
    focus: true,
...
$('#modal-content').css('width',"35em")
    $('#modal-content').html(`
  <div class="card card-secondary" id="placeholder-display">
    <div class= "card-header">
          <h3 class="card-title">Insert a Placeholder</h3>
   </div >
  <form id="placeholder-display-form">
        <div class="card-body">
            <div class="form-group row">
                <label class="col-md-4 form-label">Placeholder Type</label>
                    <div class="col-md-8">
                        <select  name="ph_type" id="ph-type" class="form-control select2" style="width: 100%;">
                            <option value="{{CREATOR_NAME}}"  selected="selected">Creator Name</option>
                            <option value="{{CREATOR_ID}}"  selected="selected">Creator ID</option>
                            <option value="{{CREATOR_IMAGE}}"  selected="selected">Creator Image</option>
                            <option value="{{EMAIL}}"  selected="selected">Email</option>
                            <option value="{{CARD_NAME}}"  selected="selected">Card Name</option>
                            <option value="{{CARD_ID}}"  selected="selected">Card ID</option>
                            <option value="{{CARD_IMAGE}}"  selected="selected">Card Image</option>
                        </select>
             </div>        
              </div>
         <div class="form-group row">
              <label for="tag" class="col-sm-4 form-label">Tag</label>
              <div class="col-sm-8">
    ...
    <div class="card-footer row">
            <div class="col-sm-6">
                <button  id="display-cancel-btn" onclick="$.fn.cancelDialog()"  class="btn btn-secondary float-left">Cancel</button>
              </div>
              <div class="col-sm-6">
                  <button  id="display-submit-btn"   class="btn btn-success float-right">Insert</button>
              </div>  
    ...
    $('#myModal').modal(modalOptions);
    $('#myModal').show();
   $('#display-cancel-btn').click((e)=>{
        e.preventDefault()
        cancelCallback()	
  })
  
  $('#display-submit-btn').click((e)=>{
      e.preventDefault()
      let placeHolder = { 'tag': $('#tag').val(), 'value': $('#ph-type').val() }
      insertCallback(elementId, placeHolder);
...
class PlaceHolder {
  ...
  static get toolbox() {
    return {
      title: 'Placeholder',
      icon: `<svg width="100%" height="100%" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4 17H20M20 17L16 13M20 17L16 21M20 7H4M4 7L8 3M4 7L8 11" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>`
    };
  }

  constructor({data}){
    this.data = data;
    this.elementID = this.data && this.data.id ? this.data.id : null;
    this.tag = this.data && this.data.tag ? this.data.tag : null;
   // console.log()
  }
  ...
  render() {
    ...
    let idLength = 10;
      for(let i=0; i< idLength; i++){
        ...
        IdBuilder.push(inventory[index])
      }
   
    let span = document.createElement('span')
    if(this.elementID){ 
        span.id = this.elementID ? this.elementID : null;
        span.innerHTML = this.data && this.data.html ? this.data.html : '';
      
    } else {
      this.elementID = IdBuilder.join('');
       span.id = this.elementID;
       $.fn.selectPlaceholder(this.elementID,
         $.fn.closeDialog
         , (elementID, placeHolder) => { 
           
              $(`#${elementID}`).html(placeHolder.value);
              $(`#${elementID}`).attr('tag',placeHolder.tag);
              $.fn.closeDialog();
            })
      }
      return span
  ...
  save(blockContent){
    //const span = blockContent.querySelector
    return {
      html: $(`#${(this.elementID)}`).html()
      , id: this.elementID
      ,tag: $(`#${(this.elementID)}`).attr('tag')
    }
...
$.fn.getTableRecords =  (objectType, serverRecords) => {
  ...
  //if (currentUser.roleID == 1) {
      ...
      if (objectType        == 'sitesettings') {
        ...
        if (dbRecords && dbRecords.length > 0) {
          ...
          records = dbRecords.map((row) => {
            ...
            row['creationDate'] =$.fn.displayDate('creationDate')
            row['lastModifiedDate'] = $.fn.displayDate('lastModifiedDate');
            return row;
      ...
      } else if (objectType == 'templates') {
          
        if (dbRecords && dbRecords.length > 0) {                     
            records = dbRecords.map((row) => {
            let deleteInfo = `\\'templates,${row.template_id}\\'`
            return {
              ...row,
             'created_datetime': $.fn.displayDate(row, 'created_datetime')
              , 'last_modified_date': $.fn.displayDate(row, 'last_modified_date'),
        ...
        if (dbRecords && dbRecords.length > 0) {                     
         ...
         if (dbRecords && dbRecords.length > 0) {                     
            ...
            if (dbRecords && dbRecords.length > 0) {                    
                records = dbRecords.map((row) => {
                  let deleteInfo = `\\'teammembers,${row.member_id}\\'`;
                  let imageUrlInfo = window.tableMap['Images'].data.filter((image) => image._id == row.image["$oid"])
                  let imageUrl = imageUrlInfo.length == 1 ? $.fn.getGoogleUrl(imageUrlInfo[0].google_url) : '';
                  return {
                    ...row,
                    'social_media': JSON.stringify(row.social_media)
                    , 'created_datetime': $.fn.displayDate(row, 'created_datetime')
                    , 'last_modified_date': $.fn.displayDate(row, 'last_modified_date')
                    , 'image':imageUrl
                    , 'Actions': `<button class="btn btn-secondary btn-md" onClick="$.fn.editTeamMember({'member_id': ${row.member_id}})">Edit</button><button class="btn btn-danger btn-md" onClick="$.fn.showConfirmDialog('Delete TeamMember', 'Are you sure that you want to delete the TeamMember with id: ${row.teammembers_id}?', '$.fn.removeRecord(${deleteInfo})' )">Delete</button>`
                    }
              })            
         }
          ...
          if (dbRecords && dbRecords.length > 0) {
            ...
            records = dbRecords.map((row) => {
              ...
              return {
                ...
                'created_datetime':  $.fn.displayDate(row, 'created_datetime')
                , 'last_modified_date':  $.fn.displayDate(row, 'last_modified_date') 
                , 'Actions': `<button class="btn btn-secondary btn-md" onClick="$.fn.editEvent({'event_id': ${row.event_id}})">Edit</button><button class="btn btn-danger btn-md" onClick="$.fn.showConfirmDialog('Delete Event', 'Are you sure that you want to delete the Event with id: ${row.event_id}?', '$.fn.removeRecord(${deleteInfo})' )">Delete</button>`
              }
            })                           
        }
  ...
  //   if (objectType == 'groupcodes') {

  //     let tableName = 'GroupCodes';
  //     let dbRecords = serverRecords && serverRecords.length > 0 ? serverRecords : Object.keys(window.tableMap).includes($.fn.capitalize(objectType)) && window.tableMap[$.fn.capitalize(objectType)].data ? window.tableMap[$.fn.capitalize(objectType)].find({ 'user_id': parseInt(currentUser.userID) }) : null;
  //           if (dbRecords && dbRecords.length > 0) {
  //                 columns = []
  //                 records = dbRecords.map((row) => {
  //                   let deleteInfo = `\\'GroupCodes,${row.code_id}\\'`;
  //                    let resultExists = window.tableMap['Results'].find({ 'group_code': row.code })
  //           let resultHtml = '';
  //           let deletebtn =`<button class="btn btn-danger btn-md" onClick="$.fn.showConfirmDialog('Delete Code', 'Are you sure that you want to delete the Code with id: ${row.code}?', '$.fn.removeRecord(${deleteInfo})' )">Delete</button>`;
 
          
  //           if (resultExists.length > 0) { 
  //              resultHtml=`<button class="btn btn-success btn-md" onClick="$.fn.showGroupResults('${row.code}')">Results</button>`
  //              deletebtn=''
  //             }
  //                   return {
  //                     ...row,
  //                     'created_datetime': Object.keys(row).indexOf('created_datetime') > -1 && Object.keys(row['created_datetime']).indexOf('$date') > -1 ? new Date(row['created_datetime']['$date']).toString() : row['created_datetime']
  //                     , 'last_modified_date': Object.keys(row).indexOf('last_modified_date') > -1 && Object.keys(row['last_modified_date']).indexOf('$date') > -1 ? new Date(row['last_modified_date']['$date']).toString() : row['last_modified_date']
  //                     , 'Actions': `${resultHtml}<button class="btn btn-secondary btn-md" onClick="$.fn.createGroupCode({'code_id': ${row.code_id}})">View</button>${deletebtn}`
                  
  //                   }
  //                 })    
  //           }
   
  //     } else   if (objectType == 'summaries') {
  //          let tableName = 'GroupCodes'
  //          let dbRecords  = []

  //           const groupCodes  = Object.keys(window.tableMap).includes(tableName) && window.tableMap[tableName].data ? window.tableMap[tableName].find({'user_id':parseInt(currentUser.userID)}) : null;
  //           let codes = groupCodes.map((codeInfo) => codeInfo.code)
            
  //     dbRecords = serverRecords && serverRecords.length > 0 ? serverRecords : Object.keys(window.tableMap).includes($.fn.capitalize(objectType)) && window.tableMap[$.fn.capitalize(objectType)].data ? window.tableMap[$.fn.capitalize(objectType)].find({ 'group_code': { "$in": codes } }) : null;
  ...
  //                     'created_datetime': Object.keys(row).indexOf('created_datetime') > -1 && Object.keys(row['created_datetime']).indexOf('$date') > -1 ? new Date(row['created_datetime']['$date']).toString() : row['created_datetime']
  //                     , 'last_modified_date': Object.keys(row).indexOf('last_modified_date') > -1 && Object.keys(row['last_modified_date']).indexOf('$date') > -1 ? new Date(row['last_modified_date']['$date']).toString() : row['last_modified_date']
  //                     , 'Actions': `<button class="btn btn-secondary btn-md" onClick="$.fn.viewSummary({'summary_id': ${row.summary_id}})">Edit</button>`
                  
  //                   }
  //                 })    
  //           }

  //     }else   if (objectType == 'details') {
  //          let tableName = 'GroupCodes'
  //          let dbRecords  = []

  //     const groupCodes = serverRecords && Object.keys(serverRecords).includes('groupcodes') ? serverRecords['groupcodes'] : Object.keys(window.tableMap).includes(tableName) && window.tableMap[tableName].data ? window.tableMap[tableName].find({ 'user_id': parseInt(currentUser.userID) }) : null;
  //     let codes = groupCodes.map((codeInfo) => codeInfo.code)
          
  //     dbRecords = serverRecords && Object.keys(serverRecords).includes('details') ? serverRecords['details'].filter((result) => codes.includes(result.group_code)) : serverRecords && serverRecords.length > 0 ? serverRecords : Object.keys(window.tableMap).includes($.fn.capitalize(objectType)) && window.tableMap[$.fn.capitalize(objectType)].data ? window.tableMap[$.fn.capitalize(objectType)].find({ 'group_code': { "$in": codes } }) : null;
  ...
  //                   return {
  //                     ...row,
  //                      'answer_map': JSON.stringify(row.answer_map)
  //                     ,'created_datetime': Object.keys(row).indexOf('created_datetime') > -1 && Object.keys(row['created_datetime']).indexOf('$date') > -1 ? new Date(row['created_datetime']['$date']).toString() : row['created_datetime']
  //                     , 'last_modified_date': Object.keys(row).indexOf('last_modified_date') > -1 && Object.keys(row['last_modified_date']).indexOf('$date') > -1 ? new Date(row['last_modified_date']['$date']).toString() : row['last_modified_date']
  //                     //, 'Actions': `<button class="btn btn-secondary btn-md" onClick="$.fn.viewDetail({'detail_id': ${row.detail_id}})">Edit</button>`
                  
  //                   }
  //                 })    
  //           }

  //   } else if (objectType == 'results') {

  //     let tableName = 'GroupCodes'
  //     let dbRecords = []
  //     const groupCodes = serverRecords && Object.keys(serverRecords).includes('groupcodes') ? serverRecords['groupcodes'] : Object.keys(window.tableMap).includes(tableName) && window.tableMap[tableName].data ? window.tableMap[tableName].find({ 'user_id': parseInt(currentUser.userID) }) : null;
  //     let codes = groupCodes.map((codeInfo) => codeInfo.code)
  //     dbRecords = serverRecords && Object.keys(serverRecords).includes('results') ? serverRecords['results'].filter((result) => codes.includes(result.group_code)) : Object.keys(window.tableMap).includes($.fn.capitalize(objectType)) && window.tableMap[$.fn.capitalize(objectType)].data ? window.tableMap[$.fn.capitalize(objectType)].find({ 'group_code': { "$in": codes } }) : null;

  //       if (dbRecords && dbRecords.length > 0) {
  //          columns = []
  //            records = dbRecords.map((row) => {
  //             let recoMap = Object.keys(row).includes('recommendation_map')?JSON.stringify(row.recommendation_map):''
  //             let viewHtml = Object.keys(row).includes('group_code') ? `$.fn.showGroupResults('${row.group_code}')` : `$.fn.showCandidateResult(${row.result_id})`;
  //           return {
  //             ...row//'recommendation_map':recoMap
  //             ,'created_datetime': Object.keys(row).indexOf('created_datetime') > -1 && Object.keys(row['created_datetime']).indexOf('$date') > -1 ? new Date(row['created_datetime']['$date']).toString() : row['created_datetime']
  //             , 'last_modified_date': Object.keys(row).indexOf('last_modified_date') > -1 && Object.keys(row['last_modified_date']).indexOf('$date') > -1 ? new Date(row['last_modified_date']['$date']).toString() : row['last_modified_date']
  //             , 'Actions': `<button class="btn btn-secondary btn-md" onClick="${viewHtml}">view</button>`
  ...
  //  if (records.length > 0) {
  //    columns = [];
  //    excludedLokiFields = excludedLokiFields.map((c)=>c.toLowerCase());
     
  //    $.fn.getColumns(records).forEach((column) => { if (!excludedLokiFields.includes(column.toLowerCase())) { let col = column.split('_').map((str) => { return $.fn.capitalize(str) }).join(' '); columns.push({ 'sTitle': col, 'data': column, 'defaultContent': '' }) } }) 
...
$.fn.removeMapRecord = (id, rowNumber) => {
   ...
   mapperTable.row
        .add([
          ...
          `<input type="text" class="form-control" name="${id}_val_${rowNumber}" id="${id}-val-${rowNumber}" value="" placeholder="">`,
           ...
           `<input class="btn btn-danger" type="button" id="${id}-${rowNumber}-rmv-bttn" value="remove" onclick="$.fn.removeMapRecord('${id}',${rowNumber})" />`
        ])
        .draw()
}

$.fn.getObjectMapper  = (id,field,value, props) =>{
    ...
    tableBody = Object.keys(props).map((key, index) => {
      ...
      let valueHtml = format != 'Dictionary' ? `<input type="text" class="form-control" name="${id}_val_${index}" id="${id}-val-${index}" value="${value}" placeholder="${value}"></input>` :
        `<textarea class="form-control" name="${id}_val_${index}" id="${id}-val-${index}" placeholder="${value}">${value}</textarea>`
          ...
          <td> <input class="btn btn-danger" type="button" id="${id}-${index}-rmv-bttn" value="remove" onclick="$.fn.removeMapRecord('${id}',${index})" /></td></tr>`
        })
	}
  ...
  return (`<table class="col-sm-10" id="${id}-table">
            	<thead>
                  <tr> 
                      ...
                      <td class="col-sm-2"><input type="button" class="btn btn-success" id="${id}-add-bttn" value="Add" onclick="$.fn.addObjectMapperRow('${id}')" /> </td> 
                  </tr>
               </thead>
...
$.fn.selectImage = (context) => {
  ...
  let imageInfo = $.fn.imageData && $.fn.imageData.length > 0 ?$.fn.getGoogleUrl( $.fn.imageData[0]["google_url"]): '';
	let  previewImageHtml = `<div id="preview-div" class="text-center"><img class="img-fluid" id="preview-image" src="${imageInfo}" alt="logo preview"/> </div>`;
...
$.fn.getTable = (objectType, results, statsIcon = "fa fa-arrows-alt") => { 
  let loadingTableData = [];
    if(objectType == 'sitesettings'){
        let settingsTable = DashboardTable({ "statsTitle": `<a href="#" onclick="$.fn.editSettings({'settings_id': 1})">Site Settings</a>`, "statsIcon": `"${statsIcon}"`, "tableData": loadingTableData, "id": objectType, "statsClass": "secondary" });
       $('#contentwrapper-node').html(`<div class="container-fluid"><div class="row">${settingsTable}</div>`);

    }else if(objectType == 'users'){
        let usersTable = DashboardTable({ "statsTitle": `<a href="#" onclick="$.fn.editusers({'users_id': 1})">Site users</a>`, "statsIcon": `"${statsIcon}"`, "tableData": loadingTableData, "id": objectType, "statsClass": "secondary" });
       $('#contentwrapper-node').html(`<div class="container-fluid"><div class="row">${usersTable}</div>`);

    } else if(objectType == 'images') { 
        let imagesTable = DashboardTable({ "statsTitle": `<a href="#" onclick="$.fn.editImage()">Images</a>`, "statsIcon": `"${statsIcon}"`, "tableData": loadingTableData, "id": objectType, "statsClass": "secondary" });
        $('#contentwrapper-node').html(`<div class="container-fluid"><div class="row">${imagesTable}</div>`);

    } else if(objectType == 'files') { 
        let filesTable = DashboardTable({ "statsTitle": `<a href="#" onclick="$.fn.editFile()">Files</a>`, "statsIcon": "fa fa-file", "tableData": loadingTableData, "id": objectType, "statsClass": "secondary" });
        $('#contentwrapper-node').html(`<div class="container-fluid"><div class="row">${filesTable}</div>`);

    } else if(objectType == 'roles') { 

        let rolesTable = DashboardTable({ "statsTitle": `<a href="#" onclick="$.fn.editRole)">Templates</a>`, "statsIcon": `"${statsIcon}"`, "tableData": loadingTableData, "id": objectType, "statsClass": "secondary" });
        $('#contentwrapper-node').html(`<div class="container-fluid"><div class="row">${rolesTable}</div>`);

    }else if(objectType == 'templates') { 

        let templatesTable = DashboardTable({ "statsTitle": `<a href="#" onclick="$.fn.editTemplate()">Templates</a>`, "statsIcon": `"${statsIcon}"`, "tableData": loadingTableData, "id": objectType, "statsClass": "secondary" });
        $('#contentwrapper-node').html(`<div class="container-fluid"><div class="row">${templatesTable}</div>`);

    }else if(objectType == 'pages') { 
       let pagesTable = DashboardTable({ "statsTitle": `<a href="#" onclick="$.fn.editPage()">Pages</a>`, "statsIcon": `"${statsIcon}"`, "tableData": [], "id": objectType, "statsClass": "secondary" });
        ...
        let teamMembersTable = DashboardTable({ "statsTitle": `<a href="#" onclick="$.fn.editTeamMember()">Team Members</a>`, "statsIcon": "fas fa-user-secret", "tableData": loadingTableData, "id": objectType, "statsClass": "secondary" });
        $('#contentwrapper-node').html(`<div class="container-fluid"><div class="row">${teamMembersTable}</div>`);
    }else if(objectType == 'mailtemplates') { 
        ...
        $('#contentwrapper-node').html(`<div class="container-fluid"><div class="row">${clientsTable}</div>`);
              ...
              let partnersTable = DashboardTable({ "statsTitle": `<a href="#" onclick="$.fn.editPartner()">Partners</a>`, "statsIcon": `"${statsIcon}"`, "tableData": loadingTableData, "id": objectType, "statsClass": "secondary" });
              $('#contentwrapper-node').html(`<div class="container-fluid"><div class="row">${partnersTable}</div>`);

      } else if(objectType == 'banners') { 

              let bannersTable = DashboardTable({ "statsTitle": `<a href="#" onclick="$.fn.editBanner()">Banners</a>`, "statsIcon": `"${statsIcon}"`, "tableData": loadingTableData, "id": objectType, "statsClass": "secondary" });
              $('#contentwrapper-node').html(`<div class="container-fluid"><div class="row">${bannersTable}</div>`);

      }else if(objectType == 'servicetypes') { 

              let servicetypesTable = DashboardTable({ "statsTitle": `<a href="#" onclick="$.fn.editservicetypes()">Service Types</a>`, "statsIcon": `"${statsIcon}"`, "tableData": loadingTableData, "id": objectType, "statsClass": "secondary" });
              $('#contentwrapper-node').html(`<div class="container-fluid"><div class="row">${servicetypesTable}</div>`);

      }else if(objectType == 'services') { 

              let serviceTable = DashboardTable({ "statsTitle": `<a href="#" onclick="$.fn.editService()">Services</a>`, "statsIcon": `"${statsIcon}"`, "tableData": loadingTableData, "id": objectType, "statsClass": "secondary" });
              $('#contentwrapper-node').html(`<div class="container-fluid"><div class="row">${serviceTable}</div>`);
 ...
 // localStorage.setItem('store_name', 'data_name'); 
  if (!Object.keys(window.columnOrder).includes(objectType)){
       $.ajax({
            ...
            crossDomain: true,
            success: (columOrder) => {
               window.columnOrder[objectType]  = columOrder;
              
               window.columnOrder[objectType].forEach((column) => { if (!excludedLokiFields.includes(column.toLowerCase())) { let col = column.split('_').map((str) => { return $.fn.capitalize(str) }).join(' '); columns.push({ 'sTitle': col, 'data': column, 'defaultContent': '' }) } }) 
...
$.fn.drawMailTemplatesTable = (results) => { 

  let templateTable = DashboardTable({ "statsTitle": `<a href="#" onclick="$.fn.editMailTemplate()">Mail Template</a>`, "statsIcon": "fa fa-envelope", "tableData": [], "id": "mailtemplate", "statsClass": "secondary" });
  $('#contentwrapper-node').html(`<div class="container-fluid"><div class="row">${templateTable}</div>`);

  const tableData = $.fn.getTableRecords('mailtemplates',results['mailtemplates']);
  const records = tableData['records'];
...
$.fn.drawCategoryTable = (results) => { 
  

  let categoriesTable = DashboardTable({ "statsTitle": `<a href="#" onclick="$.fn.editCategory()">Categories</a>`, "statsIcon": "fa fa-arrows-alt", "tableData": [], "id": "categories", "statsClass": "secondary" });
  $('#contentwrapper-node').html(`<div class="container-fluid"><div class="row">${categoriesTable}</div>`);

  const tableData = $.fn.getTableRecords('categories',results['categories']);
  const records = tableData['records'];
  const columns = tableData['columns'];
 $('#loading-row').remove();
...
$.fn.drawUserTable = (results) => { 
     
  let userRecords = results['users']
  let userTable = DashboardTable({ "statsTitle": `<a href="#" onclick="$.fn.editUser()">Administrative Users</a>`, "statsIcon": "fa fa-users", "tableData": [], "id": "user", "statsClass": "secondary" });
  $('#contentwrapper-node').html(`<div class="container-fluid"><div class="row">${userTable}</div>`);
  const tableData = $.fn.getTableRecords('users',userRecords);
  const records = tableData['records'];
  const columns = tableData['columns'];
  $('#loading-row').remove();
  $('#user').DataTable({
...
$.fn.drawAuditTrailTable = (results) => { 
        ...
        let tableText  = DashboardTable({ "statsTitle": "AuditTrail", "statsIcon": "fa fa-envelope", "statsClass": "secondary", "tableData": [], "id": "audittrail" })
...
$.fn.showTemplates= () => {
  const objectType = 'templates';
  $.fn.highlightSidebar(objectType)
  DisplayManager.lastRunFunction = `$.fn.showTemplates()`;
  DisplayManager.lastObjectType = objectType;
  
  let templates = Object.keys(window.tableMap).includes($.fn.capitalize(objectType)) && window.tableMap[$.fn.capitalize(objectType)].data ? window.tableMap[$.fn.capitalize(objectType)].data : null;
 
  if (!forceOnlineFetch && ( templates && templates.length > 0)) {
    // $.fn.drawTemplateTable({'templates':templates})
    $.fn.getTable(objectType, {'templates': templates});
  } else {
    $.ajax({
      url: `/cpanel/data/${objectType}?${objectType}={}&acky=${currentUser.acky}`,
      type: "GET",
      ...
      crossDomain: true,
      success: (results) => {
        // $.fn.drawTemplateTable(results);
        $.fn.getTable(objectType, results);
        
      }
      , error: (err) => {

        $.fn.showAlert('Templates could not be displayed', 'danger','$.fn.closeDialog()')
      }
...
$.fn.showPages= () => {
	  ...
	  $.fn.highlightSidebar(objectType)
	  DisplayManager.lastRunFunction = `$.fn.showPages()`;
	  DisplayManager.lastObjectType = objectType;
	  
	  let pages = Object.keys(window.tableMap).includes($.fn.capitalize(objectType)) && window.tableMap[$.fn.capitalize(objectType)].data ? window.tableMap[$.fn.capitalize(objectType)].data : null;
	 
	  if (!forceOnlineFetch && ( pages && pages.length > 0)) {
      //$.fn.drawPageTable({ 'pages': pages })
      $.fn.getTable(objectType, {'pages':pages});
	  } else {
		$.ajax({
		  url: `/cpanel/data/${objectType}?${objectType}={}&acky=${currentUser.acky}`,
		  type: "GET",
		  ...
		  crossDomain: true,
		  success: (results) => {
			//$.fn.drawPageTable(results);
			$.fn.getTable(objectType, results);
		  }
		  , error: (err) => {

			$.fn.showAlert('Pages could not be displayed', 'danger','$.fn.closeDialog()')
		  }
...
$.fn.showRoles = () => {
  ...
  $.fn.highlightSidebar(objectType)
  DisplayManager.lastRunFunction = `$.fn.showRoles()`;
  DisplayManager.lastObjectType = objectType;
  //$('.page-title').text($.fn.capitalize(objectType));

  let roles = Object.keys(window.tableMap).includes($.fn.capitalize(objectType)) && window.tableMap[$.fn.capitalize(objectType)].data? window.tableMap[$.fn.capitalize(objectType)].data:null;
  if (!forceOnlineFetch && ( roles && roles.length > 0) ){
   
       // $.fn.drawRoleTable({ 'roles': roles });
    $.fn.getTable(objectType, { 'roles': roles });
            ...
            $.ajax({
              ...
              crossDomain: true,
              success: (results) => {
                //$.fn.drawRoleTable(results);
                $.fn.getTable(objectType, results);
              }
              , error: (err) => {

                $.fn.showAlert('Roles could not be displayed', 'danger','$.fn.closeDialog()')
              }
...
$.fn.showSections = () => {
      ...
      $.fn.highlightSidebar(objectType)
      DisplayManager.lastRunFunction = `$.fn.showSections()`;
      DisplayManager.lastObjectType = objectType;
  
      let sections = Object.keys(window.tableMap).includes($.fn.capitalize(objectType)) && window.tableMap[$.fn.capitalize(objectType)].data? window.tableMap[$.fn.capitalize(objectType)].data:null;
      if (!forceOnlineFetch && ( sections && sections.length > 0) ){
        
            $.fn.getTable(objectType,{ 'sections': sections });
                ...
                $.ajax({
                  ...
                  , error: (err) => {
                    ...
                    $.fn.showAlert('Sections could not be displayed', 'danger','$.fn.closeDialog()')
                  }
...
$.fn.showTeamMembers = () => {
		  ...
		  $.fn.highlightSidebar(objectType)
		  DisplayManager.lastRunFunction = `$.fn.showTeamMembers()`;
		  DisplayManager.lastObjectType = objectType;
		  //$('.page-title').text($.fn.capitalize(objectType));

		  let teammembers = Object.keys(window.tableMap).includes($.fn.capitalize(objectType)) && window.tableMap[$.fn.capitalize(objectType)].data? window.tableMap[$.fn.capitalize(objectType)].data:null;
		  if (!forceOnlineFetch && ( teammembers && teammembers.length > 0) ){
		   
			   // $.fn.drawTeamMemberTable({ 'teammembers': teammembers });
			    $.fn.getTable(objectType, { 'teammembers': teammembers });
					...
					$.ajax({
					  ...
					  crossDomain: true,
					  success: (results) => {
						//$.fn.drawTeamMemberTable(results);
						$.fn.getTable(objectType, results);
					  }
					  , error: (err) => {

						$.fn.showAlert('TeamMembers could not be displayed', 'danger','$.fn.closeDialog()')
					  }
...
// $.fn.showMailboxes = () => {
//   const objectType = 'mailaccounts'
//   $.fn.highlightSidebar(objectType)
//   DisplayManager.lastRunFunction = `$.fn.showMailboxes()`;
//   DisplayManager.lastObjectType = objectType;
//   $('.page-title').text('Email Accounts')
//   let imap = window.tableMap['IMAPAccounts'] && window.tableMap['IMAPAccounts'].data ? window.tableMap['IMAPAccounts'].data : null;;
//   let gmail = window.tableMap['GMailAccounts'] && window.tableMap['GMailAccounts'].data ? window.tableMap['GMailAccounts'].data : null;
...
//   } else {
//     $.ajax({
//       url: "/cpanel/data/ImapAccounts+GmailAccounts?imapaccounts={}&gmailaccounts={}",
//       type: "GET",
//       processData: false,
//       contentType: false,
//       crossDomain: true,
...
$.fn.showMailTemplates = () => {
  ...
  $.fn.highlightSidebar(objectType);
  DisplayManager.lastRunFunction = `$.fn.showMailTemplates()`; 
  DisplayManager.lastObjectType = objectType;
  $('.page-title').text($.fn.capitalize(objectType));

    let mailtemplates = Object.keys(window.tableMap).includes($.fn.capitalize(objectType)) && window.tableMap[$.fn.capitalize(objectType)].data? window.tableMap[$.fn.capitalize(objectType)].data:null;
  if (!forceOnlineFetch && ( mailtemplates && mailtemplates.length > 0) ) {
   
        $.fn.drawRecipientTable({ 'mailtemplates': mailtemplates });
    
    } else {
            $.ajax({
              url: `/cpanel/data/${objectType}?${objectType}={}`,
              type: "GET",
              processData: false,
              contentType: false,
              crossDomain: true,
              ...
              , error: (err) => {
                ...
                $.fn.showAlert('Mail Templates could not be displayed', 'danger', '$.fn.closeDialog()')
              }
...
$.fn.showCategories = () => {
  ...
  $.fn.highlightSidebar(objectType)
  DisplayManager.lastRunFunction = `$.fn.showCategories()`;
  DisplayManager.lastObjectType = objectType;
  
  let categories = Object.keys(window.tableMap).includes($.fn.capitalize(objectType)) && window.tableMap[$.fn.capitalize(objectType)].data ? window.tableMap[$.fn.capitalize(objectType)].data : null;
 
  if (!forceOnlineFetch && ( categories && categories.length > 0)) {
    $.fn.drawCategoryTable({'categories':categories})
  } else {
    $.ajax({
      url: `/cpanel/data/${objectType}?${objectType}={}&acky=${currentUser.acky}`,
      type: "GET",
      ...
      , error: (err) => {
        ...
        $.fn.showAlert('Category could not be displayed', 'danger','$.fn.closeDialog()')
      }
...
$.fn.showSummaries  = () => {
    const objectType = 'summaries';
    $.fn.highlightSidebar(objectType)
    DisplayManager.lastRunFunction = `$.fn.showSummaries()` 
    DisplayManager.lastObjectType = objectType;

    let summaries = []
    if (currentUser.roleID == 1) {
        summaries = Object.keys(window.tableMap).includes($.fn.capitalize(objectType)) && window.tableMap[$.fn.capitalize(objectType)].data ? window.tableMap[$.fn.capitalize(objectType)].data : null;
    } else if (currentUser.roleID == 2) {

      const groupCodes = Object.keys(window.tableMap).includes($.fn.capitalize(objectType)) && window.tableMap[$.fn.capitalize(objectType)].data ? window.tableMap[$.fn.capitalize(objectType)].find({'user_id':parseInt(currentUser.userID)}) : null;
      let codes = groupCodes.map((codeInfo) => codeInfo.code)
      
        summaries = Object.keys(window.tableMap).includes($.fn.capitalize(objectType)) && window.tableMap[$.fn.capitalize(objectType)].data ? window.tableMap[$.fn.capitalize(objectType)].find({ 'group_code': {"$in": codes} }) : null;
    }

  if (!forceOnlineFetch && ( summaries && summaries.length > 0)) {
   
        $.fn.drawSummaryTable({ 'summaries': summaries });
    
   } else {
           let query = currentUser.roleID == 2 ? JSON.stringify({ "user_id": currentUser.userID }) : '{}';
            $.ajax({
              url: `/cpanel/data/${objectType}?${objectType}=${query}&acky=${currentUser.acky}`,
              type: "GET",
              processData: false,
              contentType: false,
              crossDomain: true,
              success: (results) => {
                $.fn.drawSummaryTable(results);
                
              }
              , error: (err) => {

                $.fn.showAlert('Summaries could not be displayed', 'danger','$.fn.closeDialog()')
              }
...
$.fn.showUsers = () => {
  ...
  $.fn.highlightSidebar(objectType);
  DisplayManager.lastRunFunction = `$.fn.showUsers()`;
  DisplayManager.lastObjectType = objectType;

  let users = Object.keys(window.tableMap).includes($.fn.capitalize(objectType)) && window.tableMap[$.fn.capitalize(objectType)].data? window.tableMap[$.fn.capitalize(objectType)].data:null;
  if (!forceOnlineFetch && ( users && users.length > 0)) {
   
        $.fn.drawUserTable({ 'users': users });
            ...
            $.ajax({
              ...
              , error: (err) => {
                ...
                $.fn.showAlert('User could not be displayed', 'danger',  '$.fn.closeDialog()' )
              }
...
$.fn.editSettings = (query = 'none') => {
  ...
  $.ajax({
   url: `/cpanel/data/${objectType}+images+gmailaccounts+imapaccounts?${objectType}=` + JSON.stringify(query)+`&images={}&gmailaccounts={}&imapaccounts={}&acky=${currentUser.acky}`, 
    type: "GET",
    processData: false,
    contentType: false,
    crossDomain: true,
    success: (results) => {
     ...
     // console.log(record)
      ...
      let imagePreview = '';
      const images = Object.keys(results).includes('images') && results['images'].length > 0 ? results['images'] : [{ "name": "logo", "google_url": "/static/logo_mini.png" }]
      ...
      const logoImages = images ? images.filter((image) => image.image_type.toLowerCase() == "logo") : null;
      ...
      const iconImages = images ? images.filter((image) => image.image_type.toLowerCase() == "icons") : null;
          let iconImagePreview = '';
            const iconImageOptions = iconImages.map((image) => { 
                let selected = "";
                    
                  if (record && record?.site_icon && image.google_url == record.site_icon) {
          ...
          let loginImagePreview = '';
            const loginImageOptions = loginImages.map((image) => { 
                let selected = "";
                    
                  if (record && record?.site_login && image.google_url == record.site_login) {
      ...
      $('#contentwrapper-node').html(
        `<div class="container-fluid"><div class="row">       
         ...
         <div class="card-header">
         <h3 class="card-title">${titlePrefix} Settings</h3>
         </div>
         ...
         <div class="card-body">   
          ...
          <div class="col-md-10"> 
          <div id="image-preview" class="text-center"><img  class="img-fluid" id="preview-image" src="${imagePreview}" alt="logo preview"/> </div>
          ...
          <div class="col-md-10"> 
          <div id="image-preview" class="text-center"><img  class="img-fluid" id="icon-preview-image" src="${iconImagePreview}" alt="icon preview"/> </div>
          ...
          <div class="col-md-10"> 
                <div id="image-preview" class="text-center"><img   class="img-fluid" id="login-preview-image" src="${loginImagePreview}" alt="login preview"/> </div>
		  ...
		  <div class="form-group row">
              ...
              <div class="col-sm-10">
                 <input type="text"   class="form-control"  id="secret-key" name="secret_key"  value="${record?.secret_key ? record.secret_key : ''}">
              </div>
          </div>
						<div class="form-group row">
         ...
         <div class="form-group row">
							...
							<div class="col-sm-10"> 
							${$.fn.getObjectMapper('social-media', 'Site', 'URL', record?.social_media)}
							</div>
						</div>	
						<div class="form-group row">
							<label for="overrides" class="col-sm-2 col-form-label">Overrides</label>
							<div class="col-sm-10">
								 	${$.fn.getObjectMapper('overrides', 'Parameter', 'Value', record?.overrides)}
							</div>
						</div>						
              <div class="form-group row">
      ...
      $('#settings-cancel-btn').on('click', (e) => {
       ...
       $('#overrides-table').DataTable();
        $('.select2').select2();
        $('#site-name').on('change',    (e)   => { $.fn.isFieldValid(e.target, 'settings-submit-btn', [ 'site-title', 'site-id', 'site-title', 'site-description', 'site-keywords', 'startup-message', 'secret-key', 'address','email','phone-number','time-out']) })
        $('#site-description').on('change', (e) => { $.fn.isFieldValid(e.target, 'settings-submit-btn', ['site-title', 'site-name', 'site-id', 'site-keywords', 'startup-message', 'secret-key', 'address', 'email', 'phone-number',  'time-out']) })
        $('#site-title').on('change', (e) => { $.fn.isFieldValid(e.target, 'settings-submit-btn',  ['site-name', 'site-id', 'site-title', 'site-description', 'site-keywords', 'startup-message', 'secret-key', 'address','email','phone-number', 'time-out']) }) 
        $('#site-id').on('change', (e) => { $.fn.isFieldValid(e.target, 'settings-submit-btn', ['site-title', 'site-name', 'site-description', 'site-keywords', 'startup-message', 'secret-key', 'address', 'email', 'phone-number',  'time-out']) })
        $('#site-keywords').on('change', (e) => { $.fn.isFieldValid(e.target, 'settings-submit-btn', ['site-title', 'site-name', 'site-description', 'site-id', 'startup-message', 'secret-key', 'address', 'email', 'phone-number',  'time-out']) })
        $('#startup-message').on('change', (e) => { $.fn.isFieldValid(e.target, 'settings-submit-btn', ['site-title', 'site-name', 'site-description', 'site-id', 'site-keywords', 'secret-key', 'address', 'email', 'phone-number',  'time-out']) })
        $('#secret-key').on('change', (e) => { $.fn.isFieldValid(e.target, 'settings-submit-btn', ['site-title', 'site-name', 'site-description', 'site-id', 'startup-message', 'site-keywords', 'address', 'email', 'phone-number',  'time-out']) })
        $('#address').on('change', (e) => { $.fn.isFieldValid(e.target, 'settings-submit-btn', ['site-title', 'site-name', 'site-description', 'site-id', 'startup-message', 'secret-key', 'site-keywords', 'email', 'phone-number',  'time-out']) })
        $('#email').on('change', (e) => { $.fn.isFieldValid(e.target, 'settings-submit-btn', ['site-title', 'site-name', 'site-description', 'site-id', 'startup-message', 'secret-key', 'address', 'site-keywords', 'phone-number',  'time-out']) })
        $('#phone-number').on('change', (e) => { $.fn.isFieldValid(e.target, 'settings-submit-btn', ['site-title', 'site-name', 'site-description', 'site-id', 'startup-message', 'secret-key', 'address', 'site-keywords', 'email',  'time-out']) })
        $('#time-out').on('change', (e) => { $.fn.isFieldValid(e.target, 'settings-submit-btn', ['site-title', 'site-name', 'site-description', 'site-id', 'startup-message', 'secret-key', 'address', 'email', 'phone-number',  'site-keywords','contact-us-message']) })
        $('#contact-us-message').on('change', (e) => {
       ...
       $('#google-map').on('change', (e) => {
          ...
          $('#settings-submit-btn').on('click', (e) => { 
            ...
            let socialMedia      = JSON.stringify($.fn.getObjectFromMapper('social-media'))
            let syncMode         = $('#sync-mode').val()
            let timeOut          = $('#time-out').val()
            let contactUsMessage = $('#contact-us-message').val()
            let overrides        = JSON.stringify($.fn.getObjectFromMapper('overrides'))
            ...
            let isValid = contactUsMessage.length > 0 && $.fn.areFieldsValid('settings-submit-btn', ['site-name', 'site-id', 'site-title', 'site-description', 'site-keywords', 'startup-message', 'secret-key', 'address', 'email', 'phone-number', 'time-out']) 
...
$.fn.editImage = (query = 'none') => {
  ...
  $.ajax({
    ...
    success: (results) => {
      ...
      let imagePreview = '';
      let  previewImageHtml = `<div id="preview-div" class="text-center"><img  class="img-fluid" id="preview-image" src="" alt="logo preview"/> </div>`;
      if (record && record?.google_url) { 
        ...
        previewImageHtml = `<div id="preview-div" class="text-center"><img  class="img-fluid" id="preview-image" src="${imagePreview}" width="${width}" height="${height}" alt="logo preview"/> </div>`;
      };
      ...
      $('#contentwrapper-node').html(
        `<div class="container-fluid"><div class="row">       
         <div class="card card-dark col-md-12 w-100">
         <div class="card-header">
         <h3 class="card-title">${titlePrefix} Image</h3>
         </div>
         <form class="form-horizontal">
         <div class="card-body">
      ...
      $('#image-type').on('change', (e) => {
            ...
            $('#image-cancel-btn').on('click', (e) => {
             ...
             $('#image-submit-btn').on('submit click',(e) => { 
              ...
              let isValid      = imageName.length > 0 && $.fn.isValidJSON('#image-dimensions', imageDimensions.replaceAll("\'","\""), '#image-submit-btn');
    ...
    success: (results) => {
      ...
      $('#contentwrapper-node').html(
        `<div class="container-fluid"><div class="row">       
         <div class="card card-dark col-md-12 w-100">
         <div class="card-header">
         <h3 class="card-title">${titlePrefix} File</h3>
         </div>
         <form class="form-horizontal">
         <div class="card-body">
        ...
        <div class="form-group row">
         ...
         <div class="col-sm-10"> <input type="text" name="file_name" class="form-control" id="file-name" placeholder="File Name" value="${record?.file_name ? record.file_name: ''}" ${disabled}></div>
         </div> 
				<div class="form-group row">
    ...
    success: (results) => {
      ...
      $('#contentwrapper-node').html(
        `<div class="container-fluid"><div class="row">       
         <div class="card card-dark col-md-12 w-100">
         <div class="card-header">
         <h3 class="card-title">${titlePrefix} Template</h3>
         </div>
         <form class="form-horizontal">
         <div class="card-body">
...
$.fn.editPage = (query = 'none') => {
  ...
  $.ajax({
    //url: `/cpanel/data/${objectType}+images+templates?${objectType}=` + JSON.stringify(query)+`&images={}&templates={}&acky=${currentUser.acky}`,
    ...
    let pages = Object.keys(results).includes(objectType) && results[objectType].length > 0 ? results[objectType] : null;
    let record = null;
    
      if (query && query != 'none' && Object.keys(query).length > 0) { 
       
				record  = pages.filter((rec)=>{
         
				  return rec.page_id == query.page_id
			  })
      }
      ...
      let banners = Object.keys(results).includes('banners') && results['banners'].length == 1 ? results['banners'] : null;
      $.fn.imageData = Object.keys(results).includes('images') && results['images'].length > 0 ? results['images'] : [];
      let titlePrefix = record ? 'Edit' : 'New';
      let button = record ? 'Update' : 'Add';
      ...
      let isNavOptions =  ['No','Yes'].map((opt, key) => {
        ...
        pages.splice(0, 0, {'page_id': 0, 'page_name':'None' });
          let pageOrderOptions = pages && pages.length > 0 ? pages.map((page, key) => {
            let selected = "";
      ...
      let parentPageOptions =pages && pages.length> 0? pages.filter((page)=>page.page_name != record?.page_name).map((page, key) => {
        let selected = "";
        
        if (record && record.parent_page && record.parent_page._ref && record.parent_page._ref["$id"]["$oid"] == page._id) {
            selected = `selected="selected"`
        }else if (record && record.parent_page && record.parent_page["$oid"] && record.parent_page["$oid"] == page._id) {
            selected = `selected="selected"`
      ...
      let  bannerOptions = banners && banners.length> 0? banners.map((banner, key) => {
                let selected = "";
    ...
    let recordContents =record && record.contents ? record.contents: 'Place <em>some</em> <u>text</u> <strong>here</strong>'
	  ...
	  $('#contentwrapper-node').html(
        `<div class="container-fluid"><div class="row">       
         <div class="card card-dark col-md-12 w-100">
         <div class="card-header">
         <h3 class="card-title">${titlePrefix} Page</h3>
         </div>
         <form class="form-horizontal">
         <div class="card-body">
    ...
    <div class="form-group row">
			<label  for="comes-after" class="col-md-2">Banner</label>
			<div class="col-md-10"> 
				<select name="banner_id" id="banner-id" class="form-control select2" style="width: 100%;">
	 ...
	 <div class="form-group row">
	   ...
	   <div class="form-group row">
         ...
         <div class="col-sm-10"> <input type="text" name="page_href" class="form-control" id="page-href" placeholder="page link" value="${record?.href ? record.href: ''}" ></div>
		  ...
		  <div class="form-group row">
			<label  for="is-hopme-page" class="col-md-2">Is Home Page</label>
			<div class="col-md-10"> 
				<select name="is_home_page" id="is-home-page" class="form-control select2" style="width: 100%;">
					${isHomePageOptions}
				</select>
			</div>
		</div>
     ...
     $(".form-control").on('keydown', function (e) {
      ...
      if (isValid) {
            ...
            $.ajax({
              ...
              error: (e) => {                
                  $.fn.showAlert('Page Creation Failed', 'danger', () => { $.fn.showTableRecords(objectType) })
              }
            });
...
$.fn.editTeamMember = (query = 'none') => {
  ...
  $.ajax({
    ...
    success: (results) => {
      ...
      let imageUrl = '';
      if (Object.keys(record).includes("image")){ 
        ...
        `<div class="container-fluid"><div class="row">       
              ...
              <div class="card-body">
			      <div class="form-group row">
						...
						<div class="col-md-10"> 
							<div id="image-preview" class="text-center"><img  class="img-fluid" class="rounded" id="preview-image" src="${imagePreview}" alt="Profile Image"/> </div>
    ...
    success: (results) => {
      ...
      let imagePreview    = imageUrl;
        ...
        $('#contentwrapper-node').html(
              ...
              <div class="card card-dark col-md-12 w-100">
              <div class="card-header">
              <h3 class="card-title">${titlePrefix} Banner</h3>
              </div>
              <form class="form-horizontal">
              <div class="card-body">
              <div class="form-group row">
              ...
              <div class="form-group row">
                <label for="header" class="col-sm-2 form-label">Header</label>
                <div class="col-sm-10">
                <input type="text" class="form-control" name="header" id="header" value="${record?.header ? record.header : ''}" placeholder="Header">
                </div>
              </div>
                <div class="form-group row">
                <label for="text" class="col-sm-2 form-label">Text</label>
                  <div class="col-sm-10">
                      <input type="text"   class="form-control"  id="text" name="text"  value="${record?.text ? record.text : ''}">
                  </div>
                </div>
                <div class="form-group row">
                  <label  for="image" class="col-md-2">Image</label>
                  <div class="col-md-10"> 
                    <div id="image-preview" class="text-center"><img class="rounded" id="preview-image" src="${imagePreview}" alt="Image"/> </div>
      ...
      $('.select2').select2();
      $('#name').on('change', (e) => { $.fn.isFieldValid(e.target, 'banner-submit-btn', [,'header', 'text']) })
			$('#header').on('change', (e) => { $.fn.isFieldValid(e.target, 'banner-submit-btn', ['name', 'text']) })
			$('#text').on('change', (e) => { $.fn.isFieldValid(e.target, 'banner-submit-btn', ['name','header']) })
      ...
      $('#banner-submit-btn').on('submit click',(e) => { 
        ...
        if ($.fn.areFieldsValid('banner-submit-btn', ['header', 'text'])) {
  ...
  $.ajax({
    ...
    success: (results) => {
      ...
      $('#contentwrapper-node').html(
        `<div class="container-fluid"><div class="row">       
         <div class="card card-dark col-md-12 w-100">
         <div class="card-header">
         <h3 class="card-title"><i class="fa fa-user"></i>&nbsp;&nbsp;&nbsp;${titlePrefix} User</h3>
         </div>
         <form class="form-horizontal">
         <div class="card-body">
    ...
    <label class="col-sm-2 form-label" >Connection Status </label>
      ...
      $('#user-submit-btn').on('submit click',(e) => {
        ...
        let isValid =  passwordChanged?$.fn.areFieldsValid('user-submit-bttn',['password','confirm-password']):true
        ...
        $('#contentwrapper-node').html(
              ...
              <div class="card-body">
			         <div class="form-group row">
						<label  for="profile-image" class="col-md-2">Profile Image</label>
						<div class="col-md-10"> 
							<div id="image-preview" class="text-center"><img class="rounded" id="preview-image" src="${imagePreview}" alt="Profile Image"/> </div>
						 <select name="profile_image" id="profile-image" class="form-control select2" style="width: 100%;">
								${imageOptions}
						</select>
      ...
      $('#contentwrapper-node').html(
        `<div class="container-fluid"><div class="row">       
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-10"> 
                          <div id="image-preview" class="text-center"><img class="rounded" id="preview-image" src="${imagePreview}" alt="Partner Image"/> </div>
                          <br />
              ...
              <div class="form-group row">
                ...
                <input type="text"   class="form-control"  id="trigger-description" name="trigger_description"  value="${record?.description ? record.description : ''}">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            ${$.fn.getObjectMapper('parameters', 'Name', 'Value',parameters )}
                        </div>
                    </div>		
				          <div class="form-group row">
      ...
      let  sNavItems        = {...window.defaultComponents.sidebar.sidebarNav.sidebarNavItems,sidebarNavItemList:[
         {
              ...
              "href": "#",
              "navLinkClass":"nav-link",
              "itemIconClass": "fas fa-file nav-icon",
              "text": "Files",
              "spanClass": "",
              "spanText": "",
              "position": 1
              ...
              ,"subNavItemList": [ {
                ...
                "text": "Add"
                  ...
                  "spanText": "",
                   "position": 3
                    , "actions": { "event": "onClick", "functionName": "$.fn.showTableRecords('sections')", "parameters": {} }
                   ,"subNavItemList": [ {
                      "href": "#",
                     "itemIconClass": "fa fa-plus nav-icon",
                     "text": "Add"
                      ,"actions":{"event":"onClick", "functionName":"$.fn.editSection()", "parameters":{}}
                      }
                      ]
        }
...
$.fn.loadConfigurations=()=>{
  ...
  $('.page-title').text("Site Settings");
      let  sNavItems        = {...window.defaultComponents.sidebar.sidebarNav.sidebarNavItems,sidebarNavItemList:[
       ...
       "position": 2
        ...
        ,"subNavItemList": [ {
          ...
          "itemIconClass": "fa fa-space-shuttle nav-icon",
                  ...
                  "spanText": "",
                   "position": 4
                    , "actions": { "event": "onClick", "functionName": "$.fn.showTableRecords('teammembers')", "parameters": {} }
                   ,"subNavItemList": [ {
                      "href": "#",
                     "itemIconClass": "fa fa-plus nav-icon",
                     "text": "Add"
                      ,"actions":{"event":"onClick", "functionName":"$.fn.editTeamMember()", "parameters":{}}
                      }
                      ]
        }, {
        "itemClass":"nav-item",   
       ...
       "spanText": "",
       "position": 5
          , "actions": { "event": "onClick", "functionName": "$.fn.showTableRecords('clients')", "parameters": {} }
        ,"subNavItemList": [ {
                      "href": "#",
          "itemIconClass": "fa fa-plus nav-icon",
          "text": "Add"
                      ,"actions":{"event":"onClick", "functionName":"$.fn.editClient()", "parameters":{}}
                      }
                      ]
        }, {
        "itemClass":"nav-item",   
       ...
       "spanText": "",
       "position": 6
          , "actions": { "event": "onClick", "functionName": "$.fn.showTableRecords('partners')", "parameters": {} }
        ,"subNavItemList": [ {
                      "href": "#",
          "itemIconClass": "fa fa-plus nav-icon",
          "text": "Add"
                      ,"actions":{"event":"onClick", "functionName":"$.fn.editPartner()", "parameters":{}}
                      }
                      ]
        },{
        "itemClass":"nav-item",   
      ...
      , "subNavItemList": [{
        ...
        , "subNavItemList": [{
          "href": "#",
          "itemIconClass": "fas fa-plus nav-icon",
          "text": "New Template"
          , "actions": { "event": "onClick", "functionName": "$.fn.editMailTemplate()", "parameters": {} }
                  ...
                  "href": "#",
                  "navLinkClass":"nav-link",
          "itemIconClass": "far fa-meh nav-icon",
                  "text": "Service Types",
                  "spanClass": "",
                  "spanText": "",
                   "position": 9
                    , "actions": { "event": "onClick", "functionName": "$.fn.showTableRecords('servicetypes')", "parameters": {} }
                   ,"subNavItemList": [ {
                      "href": "#",
                     "itemIconClass": "fa fa-plus nav-icon",
                     "text": "Add"
                      ,"actions":{"event":"onClick", "functionName":"$.fn.editServiceType()", "parameters":{}}
                      }
                      ]
        }
        , {
          ...
          "itemIconClass": "fa fa-graduation-cap nav-icon",
                  ...
                  "spanText": "",
                   "position": 10
                    , "actions": { "event": "onClick", "functionName": "$.fn.showTableRecords('services')", "parameters": {} }
                   ,"subNavItemList": [ {
                      "href": "#",
                     "itemIconClass": "fa fa-plus nav-icon",
                     "text": "Add"
                      ,"actions":{"event":"onClick", "functionName":"$.fn.editService()", "parameters":{}}
                      }
                      ]
        }	, {
		  "itemClass":"nav-item",   
