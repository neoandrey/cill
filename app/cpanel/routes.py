...
def cpanel_get_data(table):
    ...
    for tab in tables:
        ...
        if str(query).strip().lower() == "none":
            query = None

        data = collection.find(query) if query is not None else None
    ...
    if mode == "new":
        if data_type == "users":
            ...
            file_path = os.path.join(
                current_app.config["FILE_UPLOAD_DIRECTORY"] + os.sep + file_type.upper()
            )
            try:
            ...
            if not is_existing_template:
                template_id = Templates.get_next("template_id")
                template = Templates(
            ...
            template = None
            if banner_id:
                banner = Templates.get({"banner_id": int(banner_id)})
            ...
            description = request.form.get("description")
            partner_image = request.form.get("partner_image")
            if partner_image:
                partner_image = Images.get({"google_url": partner_image})
            partner = Partners.get({"partner_name": partner_name})
            is_existing_partner = True if partner else False
            if not is_existing_partner:
                partner_id = Partners.get_next("partner_id")
                partner = Partners(
                    partner_id=partner_id,
                    partner_name=partner_name.strip(),
                    partner_image=partner_image,
                    description=description.strip(),
                    partne_imager=partner_name.strip(),
                    last_modified_date=datetime.now(),
                    created_datetime=datetime.now(),
                    current_version=0,
                )
                ...
                return jsonify({"message": "Partner created successfully"})
            return jsonify({"message": "Partner already exists"})
        elif data_type == "servicetypes":
            type_name = request.form.get("type_name")
            type_description = request.form.get("type_description")
            service_type = ServiceTypes.get({"type_name": type_name})
            is_existing_type = True if service_type else False
            if not is_existing_type:
                type_id = ServiceTypes.get_next("type_id")
                ...
                AuditTrail.log_to_trail(
                    {
                        "old_object": None,
                        "new_object": service_type,
                        "description": f"{data_type} record updated",
                        "change_type": "INSERT",
                        "object_type": "Service Type",
                        "user_id": str(user_id),
                        "username": user_name,
                    }
                )

                return jsonify({"message": "Service Type created successfully"})
            return jsonify({"message": "Service Type already exists"})
            ...
            available_features = request.form.get("available_features")
            available_features = (
                json.loads(available_features)
                if available_features and len(available_features) > 0
                else ""
            )

            restricted_features = request.form.get("restricted_features")
            restricted_features = (
                json.loads(restricted_features)
                if restricted_features and len(restricted_features) > 0
                else ""
            )
            ...
            service_type = (
                ServiceTypes.get({"type_id": service_type})
                if bool(service_type) and service_type != "null"
                else None
            )
            page = (
                Pages.get({"page_id": page}) if bool(page) and page != "null" else None
            )
            service = Services.get({"name": name})
            is_existing_service = bool(service)
            if not is_existing_service:
                service_id = Services.get_next("service_id")
                service = Services(
            ...
            return jsonify({"message": "Service already exists"})
        ...
        if data_type == "sitesettings":
            ...
            template = None
            if banner_id:
                banner = Templates.get({"banner_id": banner_id})
            ...
            if page_check:
                ...
                page.href = href
                page.is_home_page = is_home_page
            ...
            if social_media and len(social_media) > 0:
                ...
                social_media = None
            teammember = TeamMembers.get({"member_id": member_id})
            old_object = teammember
            is_existing_teammember = True if teammember else False
            if is_existing_teammember:

                teammember.name = name.strip()
                teammember.description = description.strip()
                teammember.role = role
                teammember.social_media = social_media
                teammember.image = image
                teammember.created_datetime = datetime.now()
                teammember.last_modified_date = datetime.now()
                teammember.current_version = int(teammember.current_version) + 1
                ...
                AuditTrail.log_to_trail(
                    {
                        "old_object": old_object,
                        "new_object": teammember,
                        "description": f"{data_type} with id {member_id} record updated successfully",
                        "change_type": "UPDATE",
                        "object_type": "TeamMember",
                        "user_id": str(user_id),
                        "username": user_name,
            ...
            return jsonify({"message": "TeamMember could not be updated"})
        elif data_type == "imap":
            imap_server_address = request.form.get("imap_server_address")
            account_name = request.form.get("account_name")
            imap_username = request.form.get("imap_username")
            ...
            if is_existing_client:
                ...
                client.last_name = str(last_name).strip()
                client.email_address = email_address.strip()
                ...
                return jsonify({"message": "Client udpated successfully"})
            return jsonify({"message": "Client could not be updated"})
            credentials_file = request.files.get("crendentials")
            email_address = request.form.get("gmail_email_address")
            gmail_servers = request.form.get("gmail_server_address")
            api_key = request.form.get("gmail_api_key")
            ...
            if partner_image:
                partner_image = Images.get({"google_url": partner_image})
            partner = Partners.get({"partner_id": partner_id})
            old_object = partner
            is_existing_partner = True if partner else False
            if is_existing_partner:

                partner.name = partner_name.strip()
                partner.description = description.strip()
                partner.partner_image = partner_image
                partner.last_modified_date = datetime.now()
                partner.current_version = int(partner.current_version) + 1
            ...
            return jsonify({"message": "Partner could not be updated"})
        elif data_type == "servicetypes":
            type_id = request.form.get("type_id")
            type_name = request.form.get("type_name")
            type_description = request.form.get("type_description")
            service_type = ServiceTypes.get({"type_id": type_id})
            is_existing_servicetype = True if service_type else False
            old_object = service_type
            if is_existing_servicetype:

                service_type.type_name = type_name.strip()
                service_type.type_description = type_description.strip()
                service_type.last_modified_date = datetime.now()
                service_type.current_version = int(service_type.current_version) + 1
                service_type.save()
                user_id = (
                    session["current_user"].user_id
                    if "current_user" in session.keys()
            ...
            return jsonify({"message": "Service Type could not be updated"})
        elif data_type == "services":
            name = request.form.get("name")
            description = request.form.get("description")
            price = request.form.get("price")
            ...
            is_existing_service = True if service else False
            old_object = service
            if is_existing_service:
                ...
                service.price = price.strip()
                service.period = int(period)
                service.available_features = available_features
                service.restricted_features = restricted_features
                service.service_type = service_type
                service.page = page
                service.last_modified_date = datetime.now()
                service.current_version = int(service.current_version) + 1
                service.save()
            ...
            return jsonify({"message": "Service could not be updated"})
        elif data_type == "eventtypes":
            type_id = request.form.get("type_id")
            type_name = request.form.get("type_name")
            description = request.form.get("description")
...
def get_column_order(table):
    ...
    if table_key:
        collection = models.__dict__[table_key]
        column_order = collection.get_schema()["order"]
        column_order.append("Actions")
    return jsonify(column_order)
